    ...
    public ArrayList<Sessao> listar() throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Calendar horaFim = Calendar.getInstance();
                horaInicio.setTime(date2);
                todos.add(new Sessao(rs.getInt("id"), rs.getInt("sala"), rs.getDouble("preco_adulto"), rs.getDouble("preco_estudante"), rs.getDouble("preco_idoso"), rs.getBoolean("its3d"), rs.getBoolean("its3d"), filme, horaInicio, horaFim));
